...
Matrix createLocalK(int e,mesh &m){
    ...
    // [ A+K  G ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD, matrixnew;
    ...
    //taf ,kappa ,labda, psi_x, psi_y , delta, ita
    u_bar = m.getParameter(taf);
    ...
    //Matriz new
    float newm = - m.getParameter(delta);
    productRealMatrix(newm*Ae/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixnew);

    //Colocando submatrices en K
    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixA,matrixK,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixG, matrixnew,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    b.push_back((J*itax)/6); b.push_back((J*itax)/6); b.push_back((J*itax)/6);
