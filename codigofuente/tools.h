...
void leerMallayCondiciones(mesh &m,char *filename){
    ...
    ifstream file;
    float taf ,kappa ,labda, psi_x, psi_y , delta, ita;
    ...
    file >> taf >> kappa >> labda >> psi_x >> psi_y >> delta >> ita; 
    ...
    cout << taf << kappa << labda << psi_x << psi_y << delta << ita;

    m.setParameters((0.25)*taf, kappa , ((2)*labda)/3 , psi_x, psi_y , (0.125)*delta, ita);
